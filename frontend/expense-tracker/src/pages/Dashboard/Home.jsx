    ...
    const fetchData = async () => {
      try {
        const [incRes, expRes] = await Promise.all([
          ...
          fetch("/api/expense", { headers: { Authorization: `Bearer ${token}` } }),
        ]);
  ...
  return (
    <MainLayout>
      <div className="p-6 max-w-6xl mx-auto">
        <h1 className="text-2xl font-semibold text-[#875cf5] mb-6">Dashboard Overview</h1>
              ...
              <PieChart>
                ...
                >
                  {pieData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={["#875cf5", "#f87171", "#34d399", "#fbbf24", "#60a5fa"][index % 5]}
                    />
                  ))}
                </Pie>
                <Tooltip />
            ...
            <ChartBox title="Balance Over Time">
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={lineData}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="balance" stroke="#875cf5" strokeWidth={3} />
                </LineChart>
              </ResponsiveContainer>
            </ChartBox>
          </div>
              ...
              {recentTransactions.map((txn, idx) => (
                <li key={idx} className="py-3 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{txn.type.toUpperCase()}</p>
                  </div>
                  <span
  ...
  <div className="bg-white p-5 rounded-lg shadow-md border border-gray-100">
    ...
    <p
      className={`text-xl font-bold ${
        color === "green" ? "text-green-500" : color === "red" ? "text-red-500" : "text-[#875cf5]"
      } mt-1`}
    >
      {value}
    </p>
